...
class Tsp:
    def next_batch(self, batch_size=1):
        ...
        for b in range(batch_size):
            ...
            clu=km.kmeans(2,points)

            i=0
            one=[]
            for (px,py) in points:
                one.append((px,py,clu[i]))
                i=i+1
            X.append(one)
            #print(X)
            #X.append(points),
    ...
    def next_small_batch(self, batch_size=1):
        X, Y = [], []
        for b in range(batch_size):
            if b%200==0:
                print("preparing dataset... %s/%s" % (b, batch_size))
            points = self.generate_data(5)
            solved = self.solve_tsp_dynamic(points)
            clu=km.kmeans(2,points)
